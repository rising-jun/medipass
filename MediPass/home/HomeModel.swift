...
class HomeModel{
    ...
    public func getHealthArray() -> [HealthCellInfo]{
        ...
        arr.append(HealthCellInfo("병원마다 다른 비급여 진료비 확인 방법은?", UIImage(named: "mac1")!))
        arr.append(HealthCellInfo("어린이보험 가입 가능한 나이 만 30세?!", UIImage(named: "mac2")!))
        arr.append(HealthCellInfo("본인부담상한제 실손보험 보상 못받는 이유", UIImage(named: "mac3")!))
        arr.append(HealthCellInfo("알아두면 좋은 꿀팁 2탄! 상항별 실손청구 서류", UIImage(named: "mac4")!))
        arr.append(HealthCellInfo("바뀐 보험료 연말정산 이거 하나만 알면 끝!", UIImage(named: "mac1")!))
